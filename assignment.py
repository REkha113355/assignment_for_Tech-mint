...
def get_webdriver_instance(browser=None):
    ...
    driver = Chrome(service=ChromeService(ChromeDriverManager().install()), options=options, desired_capabilities=caps)
...
def enter_phone_number_otp(driver, creds):
    ...
    WebDriverWait(driver, 30).until(EC.invisibility_of_element_located((By.CLASS_NAME, "loader")))
    time.sleep(1)
...
def search_and_select_student(driver):
    ...
    WebDriverWait(driver, 30).until(EC.element_to_be_clickable((By.XPATH, student_result_xpath)))
...
def validate_certificate_history(driver):
    ...
    assert WebDriverWait(driver, 30).until(EC.presence_of_element_located((By.XPATH, history_entry_xpath)))
